...
@section('content')
    <div class="col-start-2 grid grid-rows-5 lg:col-span-2">
        <div class="row-start-1 p-4 flex">
            <img src="{{URL::asset('img/reserveicon.png')}}" alt="reserve" class="h-10 mt-10"><span class="font-bold font-condensed text-3xl ml-3 mt-10">Reservations</span>
        </div>
    </div>
    <form method="post" action="{{route('updatereservation')}}" class="bg-baby-blue ml-20 col-span-4 row-span-3 -mb-1 md:col-start-3 md:col-span-3 lg:col-start-4 lg:col-span-2 row-start-2 md:row-start-2 md:row-span-2 text-2xl p-5 shadow-xl rounded-md md:-mb-10 md:ml-0 md:mr-28 lg:mr-32">
        @csrf
