    ...
    Inventory System
        ...
        <div class="flex-1 p-10  font-bold">
            ...
            @csrf
                ...
                <form action="{{ route('update') }}" method="post" class="min-h-3/6  mt-8 items-center justify-center">
         @csrf
          ...
          <input type="hidden" placeholder="" name="oid" value="{{ $Info->id }}" class="w-full mt-2 mb-2 px-4 py-2 border border-black-900 rounded-md
            ...
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
              <div class="md:col-span-5">
                ...
                <input type="text" name="name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $Info->name }}" />
              </div>
              <span class="text-red-800">@error('name') {{$message}} @enderror </span>

              <div class="md:col-span-5">
                <label for="full_name">Items</label>
                <input type="text" name="order" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $Info->order }}" />
              </div>
              <span class="text-red-800">@error('order') {{$message}} @enderror </span>

              <div class="md:col-span-5">
                <label for="full_name">Data</label>
                <input type="text" name="date" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ $Info->date }}" />
              </div>
              <span class="text-red-800">@error('date') {{$message}} @enderror </span>
